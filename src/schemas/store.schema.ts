...
export const storeQuerySchema = z.object({
    ...
    page: z
        ...
        .refine((val) => val > 0, {
            message: "Page deve essere maggiore di 0",
        }),
    limit: z
        .string()
        .optional()
...
export const storeIdSchema = z.object({
    id: z
        ...
        .transform((val) => parseInt(val, 10))
        .refine((val) => val > 0, {
            message: "ID deve essere un numero positivo",
        }),
});
...
export const storeCsvRowSchema = z.object({
    ...
    nome: z
        .string({ error: "Nome non valido!" })
        .trim()
        .min(1, { error: "Nome mancante!" }),

    indirizzo: z
        .string({ error: "Indirizzo non valido!" })
        .trim()
        .min(1, { error: "Indirizzo mancante!" }),

    città: z
        .string({ error: "Città non valida!" })
        .trim()
        .min(1, { error: "Città mancante!" }),

    latitudine: z
        .string({ error: "Latitudine mancante!" })
    ...
    longitudine: z
        .string({ error: "Longitudine mancante!" })
        .transform((val) => parseFloat(val))
